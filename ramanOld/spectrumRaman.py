...
class Spectrum:
    def __init__(self, file, control_file):
        wl, I = iExtract.extract(ASSETS_DIR / file)
        _, I_ctrl = iExtract.extract(ASSETS_DIR / control_file)

        self.I_ctrl, start, end = iProcess.trim_zeros(I_ctrl)
        self.I = I[start: end]
        self.wl = wl[start: end] * PRE.n

        i = np.argmin(np.abs(self.wl - CENTER_WL))
        i_peak = iProcess.indexPeaks(self.I, distance=len(self.wl))[0]
        if i_peak < i:
    ...
    def _calc_notch_params(self):
        ...
        params, errors, r2 = iProcess.fit(self._notch, self.fcfg / PRE.T, self.norm, np.full_like(self.fcfg, ERROR_NORM), guess=guess)
        return params

...
class CentrifugeSimulation:
    ...
    def specs(self):
        xw = self.frequencyToPosition(self.f0 - self.fw / 2) - self.frequencyToPosition(self.f0 + self.fw / 2)
    ...
    def notch(self, x0, width):  # mm, mm
        ...
        f1, f2 = self.positionToFrequency(x1), self.positionToFrequency(x2)
...
if __name__ == '__main__':
    sCFG = CentrifugeSimulation(chirpRate=0.017, fFwhm=7, xFwhm=3, thetaIn=78, gratingDensity=2400, gratingDistance=30,
                                focalLength=50, tag='s')
    fCFG = CentrifugeSimulation(chirpRate=0.31, fFwhm=29, xFwhm=10, thetaIn=44, gratingDensity=1500, gratingDistance=30,
